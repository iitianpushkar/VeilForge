...
var onHttpTrigger = (runtime2, payload) => {
  ...
  const userData = decodeJson(payload.input);
  runtime2.log(`User data: ${userData}`);
  const { proof, commitment } = userData;
  const httpClient = new cre.capabilities.HTTPClient;
  const result = httpClient.sendRequest(runtime2, verifyProof(proof), consensusIdenticalAggregation())(runtime2.config).result();
  return result;
};
var verifyProof = (proof) => (sendRequester, config) => {
