...
export async function generateDepositProof(nullifier:string, secret:string, balance:string, depositor:string, deposit_amount : string) {

  const res = await fetch("/api/depositProof");
  ...
  console.log("got depositor", depositor)

  const commitment = await bb.poseidon2Hash([_nullifier, _secret,new Fr(ethers.parseUnits(balance,6))]);   // use this for merkle proof
  ...
  try {
    ...
    const input = {
      ...
      depositer: depositor.toString(),
      deposit_amount: ethers.parseUnits(deposit_amount,6).toString(),
  ...
  //  console.log("Circuit Input:", input);
    ...
    const result = {
      ...
      new_nullifier: new_nullifier.toString(),
      new_secret: new_secret.toString(),
      new_balance: ethers.formatUnits(ethers.parseUnits(balance,6) + ethers.parseUnits(deposit_amount,6),6),
    }
